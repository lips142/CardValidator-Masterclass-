...
void main() {
  final nCart = formatCreditCardNumber('4916 6418 5936 9080');
  final lastDigit = getLastDigit(nCart);
  List<int> values = [];

  List.generate(nCart.length, (index) {
    if (index.isOdd) {
      var value = nCart[index] * 1;
      values.add(sumDigits(value));
    } else {
      var value = nCart[index] * 2;
      values.add(sumDigits(value));
    }
  });
  ...
  for (var numer in values) {
    somaGeral += numer;
  }

  final resto = somaGeral % 10;
...
List<int> formatCreditCardNumber(String number) {
  return number.replaceAll(' ', '').split('').map((e) => int.parse(e)).toList();
}

int getLastDigit(List<int> values) {
